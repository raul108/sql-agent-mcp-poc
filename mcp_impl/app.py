...
<body>
    ...
    <script>
        ...
        function addMessage(text, isUser = true, isError = false) {
            ...
            // Format the response to show SQL and results nicely
            if (!isUser && !isError) {
                // Parse structured response
                contentDiv.innerHTML = text
                    .replace(/Generated SQL:/g, '<br><br><strong>Generated SQL:</strong>')
                    .replace(/✓ Safety validation passed/g, '<br><span style="color: #28a745;">✓ Safety validation passed</span>')
                    .replace(/SELECT /g, '<div class="sql-generated">SELECT ')
                    .replace(/;/g, ';</div>')
        ...
        async function sendQuery() {
            ...
            try {
                ...
                if (data.result && data.result.content && data.result.content[0]) {
                    addMessage(data.result.content[0].text, false);
                } else if (data.error) {
                    addMessage(`Error: ${data.error.message}`, false, true);
                } else {
                    addMessage(JSON.stringify(data, null, 2), false);
                }
            } catch (error) {
...
@app.route('/api/query', methods=['POST'])
def query():
    """Proxy queries to the MCP server"""
    try:
        data = request.json
        query_text = data.get('query')
